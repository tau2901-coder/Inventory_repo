...
def add_purchase_mstr(request):
    if request.method=="POST":
        form=tbl_purchase_mstr_form(request.POST)
        if form.is_valid():
            purchase = form.save(commit=False)
            purchase.username = request.user
            purchase.save()
            return redirect('/view_purchase_mstr')
    form=tbl_purchase_mstr_form()
    return render(request,"testapp/add_purchase_mstr.html",{"form":form})
...
def view_purchase_details(request):
    obj=tbl_purchase_details.objects.filter(status=True)
    return render(request,"testapp/view_purchase_details.html",{"obj":obj})
...
def add_purchase_details(request):
    ...
    if request.method == "POST":
        purchase_mstr_id = request.POST.get('purchase_mstr_id')
        ...
        errors = []
        if not purchase_mstr_id:
            errors.append("Select Invoice.")
        if not item_ids:
            errors.append("Add atleast one Item.")

        purchase = None
        if not errors:
            try:
                purchase = tbl_purchase_mstr.objects.get(id=purchase_mstr_id, status=True)
            except tbl_purchase_mstr.DoesNotExist:
                errors.append("Selected invoice is not valid.")
        ...
        if not errors:
            ...
            purchase.total_amount = (purchase.total_amount or 0) + added_amount
            purchase.save()
            return redirect('/view_purchase_deatils')

        return render(request, "testapp/add_purchase_details.html", {"items": items, "masters": masters, "errors": errors})

    return render(request, "testapp/add_purchase_details.html", {"items": items, "masters": masters})
...
def add_sales_master(request):
    if request.method =="POST":
        form=tbl_sales_master_form(request.POST)
        if form.is_valid():
            sale=form.save(commit=False)
            sale.username=request.user
            sale.save()
            return redirect('/view_sales_master')
    form=tbl_sales_master_form()
    return render(request,"testapp/add_sales_master.html",{"form":form})
...
def add_sales_full(request):
    ...
    if request.method == "POST":
        ...
        item_ids = request.POST.getlist('item_id[]')
        quantities = request.POST.getlist('quantity[]')
        FIXED_GST = 18  # hamesha 18% — client se aaya kuch bhi ho, yahi use hoga
        ...
        if not name:
            errors.append("Customer Name daalo.")
        if not item_ids:
            errors.append("Kam se kam ek item add karo.")

        # stock check (final safety — same item ek se zyada baar bhi aaya ho to combined qty check hoga)
        if not errors:
            required_qty = {}
            for i in range(len(item_ids)):
        ...
        if not errors:
            with transaction.atomic():
                ...
                sale.total_amount = added_amount
                sale.save()
    ...
    if request.method == "POST":
        ...
        if not errors:
            with transaction.atomic():
                ...
                purchase.total_amount = added_amount
                purchase.save()
...
def add_sales_details(request):
    items = item_mstr.objects.filter(status=True)
    for it in items:
        it.current_stock = get_item_available_stock(it.id)
    masters = tbl_sales_master.objects.filter(status=True)

    if request.method == "POST":
        sales_mstr_id = request.POST.get('sales_mstr_id')
        ...
        if not errors:
            try:
                sale_mstr = tbl_sales_master.objects.get(id=sales_mstr_id, status=True)
            except tbl_sales_master.DoesNotExist:
                errors.append("Selected invoice is not valid.")

        # sale karne se pehle stock check karo (same item ek se zyada baar select ho sakta hai)
        if not errors:
            required_qty = {}
            for i in range(len(item_ids)):
                required_qty[item_ids[i]] = required_qty.get(item_ids[i], 0) + int(quantities[i])
        ...
        if not errors:
            ...
            sale_mstr.total_amount = (sale_mstr.total_amount or 0) + added_amount
            sale_mstr.save()
            return redirect('/view_sales_details')

        return render(request, "testapp/add_sales_details.html", {"items": items, "masters": masters, "errors": errors})

    return render(request, "testapp/add_sales_details.html", {"items": items, "masters": masters})
...
def search_purchase_report(request):
    ...
    if request.method == "GET" and request.GET:
        if form.is_valid():
            ...
            qs = tbl_purchase_details.objects.filter(status=True)
            if item:
                qs = qs.filter(item_id=item)
            if from_date:
                qs = qs.filter(datetime__date__gte=from_date)
            if to_date:
                qs = qs.filter(datetime__date__lte=to_date)

            grouped = (
                qs.values('item_id', 'item_id__name')
                .annotate(
            ...
            obj = []
            for row in grouped:
                row['current_stock'] = get_item_available_stock(row['item_id'])
                obj.append(row)

            grand_total = round(sum(o['total_amount'] or 0 for o in obj), 2)
        else:
            obj = tbl_purchase_details.objects.none()
            grand_total = 0
    return render(
        request,
...
def search_sales_report(request):
    ...
    if request.method == "GET" and request.GET:
        if form.is_valid():
            ...
            qs = table_sales_details.objects.filter(status=True)
            if item:
                qs = qs.filter(item_id=item)
            if from_date:
                qs = qs.filter(datetime__date__gte=from_date)
            ...
            obj = table_sales_details.objects.none()
            grand_total = 0
    return render(
        request,
        "testapp/sales_report.html",
        {"form": form, "obj": obj, "grand_total": grand_total},
    )